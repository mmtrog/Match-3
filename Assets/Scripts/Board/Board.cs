...
public class Board
{
    ...
    internal void FillGapsWithNewItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                NormalItem.eNormalType choosenType = NormalItem.eNormalType.TYPE_ONE;

                List<NormalItem.eNormalType> neighbourTypes = GetNeighbourTypes(cell);
                
                foreach (var type in this.FilterType())
                {
                    if (neighbourTypes.Contains(type.Key))
                    {
                        continue;
                    }

                    choosenType = type.Key;
    ...
    internal Dictionary<NormalItem.eNormalType, int> FilterType()
    {
        Dictionary<NormalItem.eNormalType, int> typeList = new Dictionary<NormalItem.eNormalType, int>();
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            ...
            {
                Cell cell = m_cells[x, y];
                if (cell.IsEmpty || cell.Item == null || cell.Item.GetType() != typeof(NormalItem)) continue;

                bool hasTypeInDic = false;
                
                NormalItem normalItem = (NormalItem)cell.Item;
                
                foreach (var eNormalTye in typeList)
                {
                    if (normalItem.ItemType == eNormalTye.Key)
                    {
                        int temp = eNormalTye.Value + 1;

                        typeList.Remove(normalItem.ItemType);
                        
                        typeList.Add(normalItem.ItemType, temp);
                ...
                if (!hasTypeInDic)
                {
                    typeList.Add(normalItem.ItemType, 1);
                }
            }
        }

        var sortedList = typeList.ToList();
        
        sortedList.Sort((pair1, pair2) => Comparer<int>.Default.Compare(pair1.Value, pair2.Value));
        
        return sortedList.ToDictionary(pair => pair.Key, pair => pair.Value);
    }

    internal List<NormalItem.eNormalType> GetNeighbourTypes(Cell cell)
    {
        List<NormalItem.eNormalType> neighbourTypes = new List<NormalItem.eNormalType>();

        if (cell.NeighbourLeft != null && cell.NeighbourLeft.Item != null && cell.NeighbourLeft.Item.GetType() == typeof(NormalItem))
        {
            NormalItem neighbour = (NormalItem)cell.NeighbourLeft.Item;
                    
            neighbourTypes.Add(neighbour.ItemType);
        }
                
        if (cell.NeighbourRight != null && cell.NeighbourRight.Item != null && cell.NeighbourRight.Item.GetType() == typeof(NormalItem))
        {
            NormalItem neighbour = (NormalItem)cell.NeighbourRight.Item;
                    
            neighbourTypes.Add(neighbour.ItemType);
        }
                
        if (cell.NeighbourUp != null && cell.NeighbourUp.Item != null && cell.NeighbourUp.Item.GetType() == typeof(NormalItem))
        {
            NormalItem neighbour = (NormalItem)cell.NeighbourUp.Item;
                    
            neighbourTypes.Add(neighbour.ItemType);
        }
                
        if (cell.NeighbourBottom != null && cell.NeighbourBottom.Item != null && cell.NeighbourBottom.Item.GetType() == typeof(NormalItem))
        {
            NormalItem neighbour = (NormalItem)cell.NeighbourBottom.Item;
